...
const stopSocketCommand="qwerPAUSEzxcv";
var streamRunning=false;
var trendZone=document.getElementById("trendSection");
...
var inputTweet = document.getElementById('searchTweet');
var realtimeChatBox=document.getElementById("chatbox");
var overview=document.getElementById("overallstatus");
var saveSearchKeyword=null;
...
//==========================================================================get trends
var aussieTrendsContents=document.getElementById("getTrends").innerHTML;
aussieTrendsContents=aussieTrendsContents.replace(/&#34;/g,"\"");
var aussieTrendObjects = JSON.parse((aussieTrendsContents));
for(var i=0;i<aussieTrendObjects.length;i++)
{

    (function(){
        var eachTrend=aussieTrendObjects[i].name;
        var singleTrend=document.createElement("button");
        singleTrend.className="btn btn-default tooltips";
        singleTrend.type="button";
        singleTrend.innerHTML=eachTrend;
        singleTrend.onclick = function(){
            inputTweet.value=eachTrend;
        };
        trendZone.appendChild(singleTrend);
    }());
...
socket.on('getTweet', function(tweet){

    console.log("streaming: "+streamRunning);
    console.log("inputTweet.value: "+inputTweet.value);
    console.log("tweet.searchkeyword: "+tweet.searchkeyword);
    console.log(tweet.tweetcomponents);

    //if status is not paused, keep rendering
    if(streamRunning)
    {
        saveSearchKeyword=inputTweet.value;
        searchWhat.innerHTML="Search Keyword: "+inputTweet.value;
...
$( "#searchButton" ).click(function() {
    searchWhat.innerHTML="Search Keyword: "+inputTweet.value;
    socket.emit('searchTerm', inputTweet.value);
    streamRunning=true;
    //$( "#pauseButtonId" ).show();
    $( "#mainanalysis" ).hide();
    $( "#loadspinner" ).show();
...
$( "#pauseButtonId" ).click(function() {
    searchWhat.innerHTML="Now Paused: "+inputTweet.value;
    socket.emit('searchTerm', stopSocketCommand);
    streamRunning=false;
    RenderingStatusButtons();
});
$( "#resumeButtonId" ).click(function() {
    searchWhat.innerHTML="Search Keyword: "+inputTweet.value;
    socket.emit('searchTerm', inputTweet.value);
    streamRunning=true;
    RenderingStatusButtons();
...
function RenderingStatusButtons()
{
    if(streamRunning)
    {
        document.getElementById("headingstatus").innerHTML="Status: Now Running";
        //$( "#resumeButtonId" ).show();
        $( "#pauseButtonId" ).show();
        $( "#resumeButtonId" ).hide();
        //$( "#pauseButtonId" ).hide();
        ...
        document.getElementById("headingstatus").innerHTML="Status: Now Paused";
        $( "#resumeButtonId" ).show();
        //$( "#pauseButtonId" ).show();
        //$( "#resumeButtonId" ).hide();
        $( "#pauseButtonId" ).hide();
...
function RenderingOverviewControl(tweet)
{
    ...
    try{
        ...
        //headingstatus.style.cssText="display: inline;"
